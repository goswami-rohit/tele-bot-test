  ...
  async handleCompletionAction(action: string, data: any, chatIdOrSessionId: string | number, platform: 'telegram' | 'web') {
    ...
    try {
      if (action === 'create_inquiry') {
        ...
        // Retrieve the full session object for the given chatId/sessionId
        const session = platform === 'telegram' ? this.userSessions.get(chatIdOrSessionId.toString()) : this.webSessions.get(chatIdOrSessionId.toString());
        let userEmailToSave: string | undefined;

        if (session) {
          // Prioritize session.data.userEmail, then session.userEmail
          userEmailToSave = session.data?.userEmail || session.userEmail;
          if (userEmailToSave) {
            console.log(`Identified user email from session: ${userEmailToSave}`);
          } else {
            console.warn(`User email not found in session for chat ID: ${chatIdOrSessionId}. Ensure it's stored during login.`);
          }
        } else {
          console.warn(`Session not found for chat ID: ${chatIdOrSessionId}. Cannot retrieve user email.`);
        }
        ...
        const salesData = {
          ...
          userEmail: userEmailToSave || data.userEmail, // This line passes the retrieved email
        };
